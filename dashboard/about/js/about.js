...
document.getElementById("addMetric")?.addEventListener("click", function () {
  ...
  newMetric.className = "metric-item grid grid-cols-2 gap-2";
  newMetric.innerHTML = `
        <input type="text" name="metric_titles[]" placeholder="Metric Title"
        ...
        <div class="flex gap-2">
            <input type="number" name="metric_counts[]" placeholder="Count"
                class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2">
            <button type="button" class="remove-metric text-red-600 hover:text-red-700">
  ...
  // Add remove functionality to the new metric
  newMetric
...
// Edit Add Metric functionality
document
  .getElementById("editAddMetric")
  ?.addEventListener("click", function () {
    const container = document.getElementById("editMetricsContainer");
    const newMetric = document.createElement("div");
    newMetric.className = "metric-item grid grid-cols-2 gap-2";
    newMetric.innerHTML = `
        <input type="text" name="edit_metric_titles[]" placeholder="Metric Title"
            class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2">
        <div class="flex gap-2">
            <input type="number" name="edit_metric_counts[]" placeholder="Count"
                class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2">
            <button type="button" class="remove-metric text-red-600 hover:text-red-700">
                <i class='bx bx-trash text-xl'></i>
            </button>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  editButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const text = card.querySelector("p.text-gray-700").textContent.trim();
